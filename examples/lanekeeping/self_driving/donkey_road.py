...
from examples.lanekeeping.custom_types import Tuple4F
...
class DonkeyRoad(Road):
    ...
    def get_concrete_representation(self, to_plot: bool = False) -> List[Tuple4F]:
        if to_plot:
            return [(point.x, point.y, 0.60000001, self.road_width) for point in self.road_points]
        return [(point.x, 0.60000001, point.y, self.road_width) for point in self.road_points]

    def get_inverse_concrete_representation(self, to_plot: bool = False) -> List[Tuple4F]:
        if to_plot:
            return [(point.x, point.y, 0.60000001, self.road_width) for point in reversed(self.road_points)]
        return [(point.x, 0.60000001, point.y, self.road_width) for point in reversed(self.road_points)]
