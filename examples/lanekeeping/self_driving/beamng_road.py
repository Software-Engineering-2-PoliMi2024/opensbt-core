...
class BeamNGRoad(Road):
    ...
    def get_concrete_representation(self, to_plot: bool = False) -> List[Tuple4F]:
        return [(point.x, point.y, -28.0, self.road_width) for point in self.road_points]

    def get_inverse_concrete_representation(self, to_plot: bool = False) -> List[Tuple4F]:
        return [(point.x, point.y, -28.0, self.road_width) for point in reversed(self.road_points)]

    def serialize_concrete_representation(self, cr: List[Tuple4F]) -> str:
        raise NotImplemented("This method should not be needed for BEAMNG")
