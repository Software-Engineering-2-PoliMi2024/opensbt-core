...
from lanekeeping.config import BEAMNG_SIM_NAME, DONKEY_SIM_NAME, UDACITY_SIM_NAME
from lanekeeping.custom_types import GymEnv
from lanekeeping.donkey.env.donkey.config import KD_DONKEY, KI_DONKEY, KP_DONKEY
...
class AutopilotAgent(Agent):
    ...
    def predict(self, obs: np.ndarray, state: Dict) -> np.ndarray:
        if (self.env_name == DONKEY_SIM_NAME or self.env_name == UDACITY_SIM_NAME) and len(state) > 0:
            ...
            throttle = np.clip(a=1.0 - steering**2 -
                               (speed / speed_limit) ** 2, a_min=0.0, a_max=1.0)
